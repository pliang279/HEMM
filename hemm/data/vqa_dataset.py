...
class VQA(Dataset):
    def __init__(self,
        ...
        for ann in self.annotations["annotations"]:
            self.images.append(ann["image_idx"])
            self.qs.append(self.qid_to_q[ann["question_id"]])
            self.gts.append(ann[annotations])
        
    def __getitem__(self, index):
        image_id = self.images[index]
        img = Image.open(os.path.join(self.image_dir, f"COCO_val2014_000000{image_id}.jpg"))
...
class VQAEvaluator(HEMMDatasetEvaluator, EvaluatorMixin):
    def __init__(self,
                 dataset_dir,
                 model,
                 evaluate_path,
